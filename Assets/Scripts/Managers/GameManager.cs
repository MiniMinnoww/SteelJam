...
namespace Managers
{
    public class GameManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            highscoreText.text = Mathf.Approximately(hs, -1) ? "Highscore: None" : $"Highscore: {FormatTime(hs)}";
        }

        private void UpdateTimerText()
        {
            timerText.text = $"{FormatTime(timeSpent)}s";
        }
    
        // Format from float seconds to seconds:tenthOfASecond
        private static string FormatTime(float t) => $"{(int)(t / 60):00}:{(int)(t % 60):00}.{(int)((t - (int)t) * 10)}";
    }
}
